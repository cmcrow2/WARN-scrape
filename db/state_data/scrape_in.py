...
import math
from constants.urls import in_
from helpers.find_last_page import find_last_page

def get_indiana_data():
    url = find_last_page(in_)
    indiana_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Indiana"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Indiana"
    ...
    print('Indiana scrape successfull........')
    return indiana_db
