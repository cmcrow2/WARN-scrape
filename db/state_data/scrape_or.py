...
import math
from constants.urls import or_
from helpers.find_last_page import find_last_page

def get_oregon_data():
    url = find_last_page(or_)
    oregon_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Oregon"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Oregon"
    ...
    print('Oregon scrape successfull........')
    return oregon_db
