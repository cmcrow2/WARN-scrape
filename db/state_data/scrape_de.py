...
import math
from constants.urls import de
from helpers.find_last_page import find_last_page

def get_delaware_data():
    url = find_last_page(de)
    delaware_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Delaware"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Delaware"
    ...
    print('Delaware scrape successfull........')
    return delaware_db
