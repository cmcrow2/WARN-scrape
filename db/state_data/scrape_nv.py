...
import math
from constants.urls import nv
from helpers.find_last_page import find_last_page

def get_nevada_data():
    url = find_last_page(nv)
    nevada_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Nevada"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Nevada"
    ...
    print('Nevada scrape successfull........')
    return nevada_db
