...
import math
from constants.urls import ne
from helpers.find_last_page import find_last_page

def get_nebraska_data():
    url = find_last_page(ne)
    nebraska_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Nebraska"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Nebraska"
    ...
    print('Nebraska scrape successfull........')
    return nebraska_db