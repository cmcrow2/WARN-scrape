...
import math
from constants.urls import ks
from helpers.find_last_page import find_last_page

def get_kansas_data():
    url = find_last_page(ks)
    kansas_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Kansas"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Kansas"
    ...
    print('Kansas scrape successfull........')
    return kansas_db
