...
import math
from constants.urls import ok
from helpers.find_last_page import find_last_page

def get_oklahoma_data():
    url = find_last_page(ok)
    oklahoma_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Oklahoma"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Oklahoma"
    ...
    print('Oklahoma scrape successfull........')
    return oklahoma_db
