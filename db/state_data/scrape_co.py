...
import math
from constants.urls import co
from helpers.find_last_page import find_last_page

def get_colorado_data():
    url = find_last_page(co)
    colorado_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Colorado"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Colorado"
    ...
    print('Colorado scrape successfull........')
    return colorado_db
