...
import math
from constants.urls import mt
from helpers.find_last_page import find_last_page

def get_montana_data():
    url = find_last_page(mt)
    montana_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Montana"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Montana"
    ...
    print('Montana scrape successfull........')
    return montana_db