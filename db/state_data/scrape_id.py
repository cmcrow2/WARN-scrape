...
import math
from constants.urls import id
from helpers.find_last_page import find_last_page

def get_idaho_data():
    url = find_last_page(id)
    idaho_db = []
    ...
    while True:
        ...
        for idx in reversed(warn_data["Reporting State"]):
            if (warn_data["Reporting State"][idx] == "Idaho"):
                temp_data = {}
                temp_data['id'] = count
                count += 1
                temp_data["state"] = "Idaho"
    ...
    print('Idaho scrape successfull........')
    return idaho_db
